<template>
    ...
    <li v-for="(item, index) in collection" :key="index" class="my-1 relative">
        ...
        <div class="parent flex items-center bg-white shadow rounded border p-2" draggable="true">
            <div v-if="! lastChild">
                <svg v-if="item.subcategories.length" @click="itemClickHandler" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 cursor-pointer transform rotate-180 text-blue-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
