        ...
        <form @submit.prevent="submit">
            <div class="w-full mx-auto max-w-4xl bg-white p-4 rounded">
                ...
                <div class="py-4 flex flex-col justify-center items-center gap-1">
                    <span v-for="(message, index) of messages" :key="index"
                        class="text-red-600 text-sm bg-red-200 px-3 py-1 rounded inline-flex gap-2 items-center">
                        <span>{{ message }}</span>
                ...
                <table class="w-full" v-show="priceType">
                    ...
                    <tbody>
                        <tr class="border-t" v-for="(saleableProduct, index) in saleableProducts" :key="index">
                            ...
                            <td class="border-r">
                                <select class="w-full border-0" @change="selectProduct(index)"
                                    v-model="saleableProduct.productId">
                                    <option v-for="(product, productId) in products" :key="productId"
                                        :disabled="product.disabled" :value="productId">
                                        {{ product.name }}
                                    </option>
                                </select>
                            </td>
                            <td class="text-right border-r">
                                <input v-show="saleableProduct.productId" type="number"
                                    class="w-20 ml-auto text-center border-0" @change="checkQuantity(index)"
    ...
    export default {
        ...
        methods: {
            ...
            checkQuantity(numberOfIndex) {
                ...
                this.saleableProducts[numberOfIndex].quantity = (product.maxQuantity >= quantity) ? quantity : product
                    .maxQuantity;
