       ...
       <div class="w-full mx-auto">
        ...
        <div class="grid grid-cols-2 gap-2">
                <div class="bg-white border shadow rounded p-4">
                   <Label class="text-center text-xl" value="Cost Category"/>
                   <add-new-button :href="route('cost-categories.create')" />
                    <data-table
                        :collections="costCategories"
                        :filters="filters"
                        :columns="columns1"
                        :latest="true">
                        <template #default="{ item: costCategory }">
                            <td class="py-3 px-2 text-left">{{ costCategory.name }}</td>
                            <td class="py-3 px-2 text-left">{{ costCategory.active == 1 ? 'Yes':'No' }}</td>
                            <td class="flex justify-center items-center">
                                <action-button-edit :href="route('cost-categories.edit', costCategory.id)" />
                                <action-button-delete :href="route('cost-categories.destroy', costCategory.id)" /> 
                            </td>
                        </template>
                    </data-table>
                </div>
                <div class="bg-white border shadow rounded p-4">
                    <Label class="text-center text-xl" value="Printing Details Category"/>
                ...
                <div class="bg-white border shadow rounded p-4">
                    <Label class="text-center text-xl" value="Binding Type"/>
                    <add-new-button :href="route('binding-types.create')" />
                      <data-table
                        :collections="bindingTypes"
                        :filters="filters"
                        :columns="columns2"
                        :latest="true">
                        <template #default="{ item: bindingType }">
                            <td class="py-3 px-2 text-left">{{ bindingType.name }}</td>
                            <td class="py-3 px-2 text-left">{{ bindingType.active == 1 ? 'Yes':'No' }}</td>
                            <td class="flex justify-center items-center">
                                <action-button-edit :href="route('binding-types.edit', bindingType.id)" />
                                <action-button-delete :href="route('binding-types.destroy', bindingType.id)" /> 
                            </td>
                        </template>
                    </data-table>
                </div>
